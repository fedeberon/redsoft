...
class SearchBarComponent extends React.Component {
    ...
    constructor(props) {
        ...
        this.formatTextSearch = this.formatTextSearch.bind(this);
    }

    formatTextSearch () {
        let newText;
        if(this.state.textSearch.endsWith('es') && this.state.textSearch !== 'cables' && this.state.textSearch !== 'mouses' ){
            newText = this.state.textSearch.slice(0, -2);
            this.setState({textSearch: newText});
            window.location.href=`/products/search=${newText}`
        } else if(this.state.textSearch.charAt(this.state.textSearch.length -1) === 's'){
            newText = this.state.textSearch.slice(0, -1);
            this.setState({textSearch: newText});
            window.location.href=`/products/search=${newText}`
        } else {
            window.location.href=`/products/search=${this.state.textSearch}`
        } 
    }
        ...
        return <div className="col-sm-4">
            <div className="input-group md-form form-sm pl-0 searchtop">
                <input id="input-user" className="form-control my-0 py-1 searchtop-input" type="text"
                       ...
                       onKeyPress={(e) => e.key === 'Enter' ? this.formatTextSearch() : ""}
                ...
                <div className="input-group-text searchtop-icon" id="basic-text1">
                    <a id="asd" href="#" onClick={() => this.formatTextSearch()} style={{
